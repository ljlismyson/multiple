"""Diffraction-multiple attenuation with sinusoidal patch-position channels.

Windows:
    conda run -n segy python scripts/coherent_noise_attenuation/train_diffraction_multiples_atten_unet_sinposenc.py \
        --config configs/coherent_noise_attenuation/diffraction_multiples_atten_unet_sinposenc.yaml
Linux/DDP:
    torchrun --nproc_per_node=2 scripts/coherent_noise_attenuation/train_diffraction_multiples_atten_unet_sinposenc.py \
        --config configs/coherent_noise_attenuation/diffraction_multiples_atten_unet_sinposenc.yaml
"""
...
def _build_denoise_patch_pairs_from_pair(
    ...
    *,
    ...
    tuple
        ...
        ``(P, 5, H, W)`` and ``(P, 1, H, W)``. Input channels are seismic,
        sin/cos trace coordinate, and sin/cos time coordinate.
    """
    ...
    input_patches = append_sinusoidal_position_channels(
        input_patches,
        input_info,
        frequencies=prep.get("position_encoding_frequencies", [1.0]),
    )
