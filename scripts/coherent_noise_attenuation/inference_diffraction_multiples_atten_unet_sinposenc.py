"""Attention U-Net inference with sinusoidal patch-position channels.

Example:
    python scripts/coherent_noise_attenuation/inference_diffraction_multiples_atten_unet_sinposenc.py \
        --config configs/coherent_noise_attenuation/diffraction_multiples_atten_unet_sinposenc.yaml \
        --checkpoint results/diffraction_multiples_atten_unet_sinposenc/checkpoints/epoch_0199.pt
"""
...
from tools.sinusoidal_position_encoding import append_sinusoidal_position_channels  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/coherent_noise_attenuation/diffraction_multiples_atten_unet_sinposenc.yaml",
...
def _inference_on_shots_sinposenc(
    ...
    *,
    ...
    patches = append_sinusoidal_position_channels(
        patches,
        info,
...
def main() -> None:
    ...
    start = time.time()
    pred_norm = _inference_on_shots_sinposenc(
        model=model,
        ...
        frequencies=[float(v) for v in prep.get("position_encoding_frequencies", [1.0])],
    )
